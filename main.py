...
class MyFrame(wx.Frame):
    def __init__(self, parent, id):
        wx.Frame.__init__(self, parent, id, '选取文件', size=(600, 400))
        # 创建面板
        panel = wx.Panel(self)
        ...
        self.title = wx.StaticText(panel,
                                   label="1、a1-10.txt    2、b1-10.txt    3、c1-10.txt    4、d1-10.txt\n")
        self.label_file = wx.StaticText(panel, label="选取文件（1~4）：")
    ...
    def OnclickSubmit(self, event):
        ...
        if self.text_file.GetValue() == '':  # 判断文件序号是否为空
            ...
            global i
            if (fileNum == 1):
                with open('a1-10.txt') as f:
                    for line[i] in f:
                        i = i + 1
            if (fileNum == 2):
                with open('b1-10.txt') as f:
                    for line[i] in f:
                        i = i + 1
            if (fileNum == 3):
                with open('c1-10.txt') as f:
                    for line[i] in f:
                        i = i + 1
            if (fileNum == 4):
                with open('d1-10.txt') as f:
                    for line[i] in f:
                        i = i + 1
            message = '读取成功'
        else:
